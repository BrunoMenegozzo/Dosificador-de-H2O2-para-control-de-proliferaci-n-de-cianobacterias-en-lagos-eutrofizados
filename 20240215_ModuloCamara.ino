...
Cam cam;
JpegDecoder decoder;
Applications::ColorBlobDetector detector(160, 185, 100);
...
// Se define el Timer 0
#define TIMER_NUMBER 0
// Tiempo en segundos para la interrupción
...
void setup() {
  ...
  //wiFiManager.resetSettings(); //Para resetear los valores almacenados de SSID y contraseña wifi. Solo esta para DEBUG y testing
  webServerConfig();
  camConfig();
  espNowConfig();
...
void loop(){
  ...
  if (isTimerInterruptTriggered) {
    ...
    //---------------------------------------------------------------
    int R, G, B;
    YCbCrToRGB(colorParam1, colorParam2, colorParam3, R, G, B);
    Serial.println("Colores en RGB son: R:");
    Serial.print(R);
    Serial.println("G:");
    Serial.print(G);
    Serial.println("B:");
    Serial.print(B);
    detector.updateColor(R,G,B);
    ...
    //-----------------------------------CIANOSEMAFORO------------------------------------------------
    float limiteVerdeAbs = LIMITE_VERDE * 48 ; // Regla de 3 simple para pasar de porcentaje a valor absoluto
    float limiteAmarilloAbs = LIMITE_AMARILLO * 48 ; // Regla de 3 simple para pasar de porcentaje a valor absoluto
    ...
    // Valor a enviar al modulo Actuador
...
void stringToHexArray(const String &hexString, uint8_t *hexArray) {
  for (int i = 0; i < 6; ++i) {
    ...
    int charIndex = i * 2;
    // Extrae el par de caracteres de la cadena y convertirlo a un valor hexadecimal
